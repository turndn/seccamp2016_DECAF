...
static void IsDebuggerPresent_call(void *opaque)
{
	...
	DECAF_printf("URL string address: 0x%x\n", ctx->call_stack[2]);
	char url_buffer[256];
...
static void geteip_loadmainmodule_callback(VMI_Callback_Params* params)
{
        if(strcmp(params->cp.name,targetname) == 0)
        {
                ...
                isdebuggerpresent_handle = hookapi_hook_function_byname("Urlmon.dll", "URLDownloadToFileW", 1, target_cr3, IsDebuggerPresent_call, NULL, 0);
